...
void create_model(size_t input_size, size_t bits_per_input, size_t num_classes, size_t filter_inputs, size_t filter_entries, size_t filter_hashes) {
    ...
    model_init(&global_model, num_inputs, num_classes, filter_inputs, filter_entries, filter_hashes, bits_per_input, 1, 0);
}

void set_hashes(uint16_t* hash_values, size_t size) {
    for(size_t it = 0; it < size; ++it)
        global_model.hash_parameters.data[it] = hash_values[it];
}

void set_ordering(uint16_t* ordering, size_t size) {
    for(size_t it = 0; it < size; ++it)
        global_model.input_order[it] = ordering[it];
}

void fill_model(uint16_t* data, size_t size) {
    for(size_t it = 0; it < size; ++it) 
        global_model.filters.data[it] = data[it];
}

size_t predict(uint8_t* input) {
